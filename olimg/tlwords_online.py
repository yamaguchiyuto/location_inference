...
class TLWords:
    def __init__(self, users, tweets, model, lwords):
        self.users = users
        ...
        for user in self.users.iter():
            if user['location_point'] == None:
                ...
                if user['id'] in self.user_distributions and len(self.user_distributions[user['id']]) > 0:
                    ...
                    result[user['id']]['location_point'] = inferred_location
                else:
                    if params['default']:
                        """ no clues            """
                        """ predict using prior """
    ...
    def infer(self, params):
        ...
        for tweet in self.tweets.stream():
            ...
            if type(tweet) == type({}) and 'timestamp' in tweet:
                ...
                if tweet_count - dump_time >= params['dump_span']:
                    dump_time = tweet_count 
                    yield (tweet_count, self.predict_all(params))
...
if __name__ == '__main__':
    ...
    tl = TLWords(users, tweets, model, lwords)
    ...
    print tl.extract_local_words_batch({'dmin':0.01, 'cmin':30, 'window_size':1800, 'tl':False, 'default':False, 'divergence':'l2'})

...
class TLWords:
    ...
    def predict(self, user_distribution, params):
        alpha = numpy.array(self.model.weights_) * params['r']
        for k in user_distribution:
            alpha[int(k)] += user_distribution[k]
        return alpha.argmax()
    ...
    def infer(self, params):
        ...
        for user in self.users.iter():
            if user['location_point'] == None:
                """ unlabeled user """
                if user['id'] in self.user_distributions:
...
if __name__ == '__main__':
    ...
    print tl.extract_local_words(tl.tweets.stream(), {'dmin':0.05, 'cmin':30, 'window_size':1800, 'tl':False, 'default':False, 'divergence':'l2'})
